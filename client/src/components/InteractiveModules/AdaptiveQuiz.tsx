...
export function AdaptiveQuiz({ language }: AdaptiveQuizProps) {
      ...
      >
        <div className="text-center">
          ...
          <h3 className="text-2xl font-bold text-card-foreground mb-4">
            <FormattedMessage
              id="quiz.finished"
              defaultMessage="Quiz ukończony!"
            />
          </h3>
          <p className="text-lg text-muted-foreground mb-6">
            <FormattedMessage
              id="quiz.score"
              defaultMessage="Twój wynik: {score} z {total} ({percent}%)"
              values={{
                score,
          ...
          >
            <FormattedMessage
              id="quiz.restart"
              defaultMessage="Rozpocznij ponownie"
            />
          </Button>
        </div>
      </div>
  ...
  return (
    <div
      className="interactive-module rounded-xl p-8"
      data-testid="adaptive-quiz-module"
    >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="px-3 py-1 bg-accent text-accent-foreground rounded-full text-xs font-medium">
                {question.difficulty === "easy" ? (
                  <FormattedMessage
                    id="quiz.difficulty.easy"
                    defaultMessage="Łatwy"
                  />
                ) : question.difficulty === "medium" ? (
                  <FormattedMessage
                    id="quiz.difficulty.medium"
                    defaultMessage="Średni"
                  />
                ) : (
                  <FormattedMessage
                    id="quiz.difficulty.hard"
                    defaultMessage="Trudny"
                  />
                )}
              </span>
            </div>
            ...
            <div className="space-y-3">
              {question.options.map((option) => (
                <Button
                  ...
                  aria-label={`Wybierz odpowiedź ${option.id.toUpperCase()}: ${getOptionText(
                    option
                  )}`}
                  role="radio"
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-muted-foreground">
                    <FormattedMessage
                      id="quiz.topic.electricity"
                      defaultMessage="Elektryczność"
                    />
                  </span>
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-muted-foreground">
                    <FormattedMessage
                      id="quiz.topic.magnetism"
                      defaultMessage="Magnetyzm"
                    />
                  </span>
            ...
            <div className="mt-6 p-4 bg-muted rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-foreground">
                  <FormattedMessage
                    id="quiz.achievement"
                    defaultMessage="Osiągnięcie"
                  />
                </span>
              </div>
              <p className="text-xs text-muted-foreground">
                <FormattedMessage
                  id="quiz.achievementText"
                  defaultMessage="Ukończono pierwszy rozdział o prądzie elektrycznym!"
                />
              </p>
            </div>
          </div>
